...
# src = "C:\PostgreSQL\16\bin" #自分で変更する
src = R""
...
#　自分で別途変更が必要　GitHubに挙げないならべた付けでもよいが、セキュリティ的にはよくない

# config,iniから値取得（.iniのファイルを本番用に書き換える）
...
cur = connection.cursor()#接続情報を受け取る
...
def csv_to_db(src):
    ...
    for cp in csv_path:
        ...
        with open(cp, newline = '', encoding='utf-8') as cpf:
            print(cpf)
